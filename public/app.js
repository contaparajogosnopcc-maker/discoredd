// ============================================================
// Discord Site - Chat + Admin + Call (CORRIGIDO)
// ============================================================
...
function connectWS() {
  ...
  ws.onmessage = (e) => {
    ...
    if (m.type === 'user_kicked') { addSystemMsg('Admin removeu um usuario'); }
    if (m.type === 'user_banned') { addSystemMsg('Admin baniu um usuario'); }
...
function renderOnline(list) {
  ...
  if (!list || !list.length) { c.innerHTML = '<div class="loading">Ninguem online</div>'; if(cnt) cnt.textContent = ''; return; }
  c.innerHTML = list.map(function(p) {
    var av = p.avatar ? 'https://cdn.discordapp.com/avatars/' + p.userId + '/' + p.avatar + '.png?size=32' : 'https://cdn.discordapp.com/embed/avatars/0.png';
    var isMe = user && user.id === p.userId;
    var adminClick = user && user.isAdmin && !isMe;
    return '<div class="online-item' + (adminClick ? ' admin-clickable' : '') + '" data-userid="' + p.userId + '" data-username="' + (p.globalName||p.username) + '"' + (adminClick ? ' onclick="openAdminModal(\'' + p.userId + '\',\'' + (p.globalName||p.username) + '\')"' : '') + '><img src="' + av + '" class="online-av"><span class="online-name">' + (p.globalName||p.username) + (isMe?' (voce)':'') + (p.isAdmin?' <span class="admin-badge-sm">ADMIN</span>':'') + '</span>' + (!isMe ? '<button class="call-btn" onclick="event.stopPropagation();startCall(\'' + p.userId + '\')" title="Ligar">📞</button>' : '') + '<span class="online-dot"></span></div>';
  }).join('');
...
async function startCall(targetId) {
  ...
  try {
    ...
    alert('Erro: Verifique se seu microfone esta disponivel.');
    console.error(e);
    endCall(false);
  }
}
