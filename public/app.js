...
const ICE = { iceServers: [{ urls: 'stun:[stun.l.google.com:19302](https://stun.l.google.com:19302)' }, { urls: 'stun:[stun1.l.google.com:19302](https://stun1.l.google.com:19302)' }, { urls: 'turn:[openrelay.metered.ca:443](https://openrelay.metered.ca:443)', username: 'openrelayproject', credential: 'openrelayproject' }] };
...
ws = new WebSocket(proto + '//' + [location.host](https://location.host) + '/?token=' + (getToken() || ''));
...
document.getElementById('themeBtn').onclick = openSettings;
...
if (!text) return;
// Comandos admin
if (text.startsWith('/') && user.isAdmin) { handleCommand(text); input.value = ''; return; }
input.value = '';
...
if (cmd === '/clear') adminClearChat();
else if (cmd === '/ban' && parts[1]) { api('/api/admin/ban/' + parts[1], { method: 'POST' }); addSystemMsg('Usuario banido: ' + parts[1]); }
else if (cmd === '/kick' && parts[1]) { api('/api/admin/user/' + parts[1], { method: 'DELETE' }); }
}
...
div.style.color = m.nameColor || '#fff';
...
content += `<button class="report-btn" onclick="reportMessage('${m.id}')" title="Denunciar">🚩</button>`;
...
const av = p.avatar ? `https://cdn.discordapp.com/avatars/${p.userId}/${p.avatar}.png?size=64` : '[https://cdn.discordapp.com/embed/avatars/0.png](https://cdn.discordapp.com/embed/avatars/0.png)';
const isMe = user?.id === p.userId;
const adminClick = user?.isAdmin && !isMe;
const clickHandler = adminClick ? `onclick="openAdminModal('${p.userId}','${(p.globalName||p.username).replace(/'/g,"\\'")}')"` : '';
return `<div class="online-item ${adminClick?'admin-target':''}" ${clickHandler}>
      <img src="${av}" class="online-av" loading="lazy">
      <span class="online-name" style="color:${p.nameColor||'#fff'}" onclick="openProfile('${p.userId}')">${escapeHtml(p.globalName||p.username)} ${isMe?'(você)':''}</span>
...
modal.innerHTML = `<h3>🛡️ Gerenciar ${uname}</h3>
    <div class="modal-actions">
      <button class="btn-warning" id="modalKick">👢 Kick</button>
      <button class="btn-danger" id="modalBan">🔨 Banir</button>
      <button class="btn-call" id="modalCall">📞 Ligar</button>
      <button class="btn-warning" onclick="assignRole('${uid}')">👥 Cargo</button>
    </div><button class="btn-cancel" onclick="hideModals()">Cancelar</button>`;
...
// Buscar perfil via API ou usar dados locais
...
modal.innerHTML = `<h3>👤 ${escapeHtml(name)}</h3>
      <p>Nível: ${rank?.level||1} | XP: ${rank?.xp||0}</p>
      <p>Rank: #${ranking.findIndex(p=>p.userId===uid)+1 || 'N/A'}</p>
      <button class="btn-cancel" onclick="hideModals()">Fechar</button>`;
...
document.getElementById('settingsModal').innerHTML = `
    <h3>🎨 Personalizar</h3>
    <label>Apelido: <input id="setNickname" value="${p.nickname||''}" maxlength="32"></label>
    <label>Cor do Nome: <input type="color" id="setNameColor" value="${p.nameColor||'#ffffff'}"></label>
    <label>Cor da Bolha: <input type="color" id="setBubbleColor" value="${p.bubbleColor||'#21262d'}"></label>
    <label>Wallpaper (URL): <input id="setWallpaper" value="${p.wallpaper||''}" placeholder="https://..."></label>
    <label>Bio: <textarea id="setBio" maxlength="200">${p.bio||''}</textarea></label>
    <button class="btn-discord" onclick="saveSettings()">Salvar</button>
    <button class="btn-cancel" onclick="hideModals()">Cancelar</button>`;
...
modal.innerHTML = `<h3>🏆 Ranking</h3><div class="ranking-list">${list.map((p,i) => `<div class="rank-item"><span>#i+1</span><spanstyle="color:{i+1}</span><span style="color:i+1</span><spanstyle="color:{p.nameColor||'#fff'}">p.nickname∣∣p.userId</span><span>Lv.{p.nickname||p.userId}</span><span>Lv.p.nickname∣∣p.userId</span><span>Lv.{p.level} (${p.xp} XP)</span></div>`).join('')}</div><button class="btn-cancel" onclick="hideModals()">Fechar</button>`;
}

function openRoleManager() {
const modal = document.getElementById('roleManagerModal');
let html = '<h3>👥 Cargos</h3><div id="roleList"></div>';
html += '<input id="newRoleName" placeholder="Nome do cargo"><input type="color" id="newRoleColor" value="#ffffff"><button onclick="createRole()">Criar</button>';
html += '<button class="btn-cancel" onclick="hideModals()">Fechar</button>';
...
if (!name) return;
await api('/api/admin/roles', { method:'POST', body: JSON.stringify({ name, color, permissions:[] }) });
renderRoles();
}
...
const roleId = prompt('ID do cargo (ou deixe vazio para remover):');
...
modal.innerHTML = `<h3>🚩 Denúncias</h3>${reports.map(r=>`<div class="report-item"><p>Mensagem: r.messageId−{r.messageId} - r.messageId−{r.reason}</p><p>Por: r.reporterName{r.reporterName} r.reporterName{r.resolved?'✅':''}</p>{r.id}')">Resolver</button>`:''}</div>`).join('')}<button class="btn-cancel" onclick="hideModals()">Fechar</button>`;
...
modal.innerHTML = `<h3>📋 Logs</h3>${logs.map(l=>`<p>newDate(l.timestamp).toLocaleString()−{new Date(l.timestamp).toLocaleString()} - newDate(l.timestamp).toLocaleString()−{l.admin} l.action{l.action} l.action{l.target} ${l.details}</p>`).join('')}<button class="btn-cancel" onclick="hideModals()">Fechar</button>`;
showModal('logsModal');
}

function reportMessage(msgId) {
const reason = prompt('Motivo da denúncia:');
if (reason) api('/api/report', { method:'POST', body: JSON.stringify({ messageId: msgId, reason }) });
}

// ==================== CALL ====================
async function startCall(targetId) { /* Mantida a lógica anterior */ }
async function handleCallOffer(m) { /* Mantida */ }
async function handleCallAnswer(m) { /* Mantida */ }
async function handleCallICE(m) { /* Mantida */ }
function endCall(notify) { /* Mantida */ }
// ... (incluir as funções de chamada do código anterior)
