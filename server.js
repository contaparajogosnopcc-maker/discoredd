...
app.get('/callback', async (req, res) => {
  ...
  try {
    ...
    if (!guilds.some(g => g.id === REQUIRED_GUILD_ID)) {
      return res.send(accessDenied('Você não está no servidor necessário.'));
    }
    ...
    if (!memberRes.ok) return res.send(accessDenied('Não foi possível verificar seus cargos.'));
    const member = await memberRes.json();
    if (!member.roles.includes(REQUIRED_ROLE_ID)) {
      return res.send(accessDenied('Você não possui o cargo necessário.'));
    }
    ...
    if (bannedUsers.has(ud.id)) return res.send(html('Banido','Você foi banido.'));
    ...
    res.send(html('Erro','Algo deu errado.'));
  }
});
...
app.post('/api/presence', (req, res) => {
  ...
  presences.set(u.id, {
    ...
    presence: { details:'No chat', state:'Conversando', startTimestamp: Date.now() },
    lastUpdate: Date.now()
  });
    ...
    try {
      ...
      if (['call_offer','call_answer','call_ice','call_ended','call_declined'].includes(data.type)) {
        if (data.target) {
          ...
          const fromName = session ? (session[1].global_name || session[1].username) : 'Usuario';
          const targetWs = userWsMap.get(data.target);
          if (targetWs && targetWs.readyState === WebSocket.OPEN) {
            targetWs.send(JSON.stringify({...data, from: userId, fromName}));
          }
        }
      }
    } catch(e) {}
